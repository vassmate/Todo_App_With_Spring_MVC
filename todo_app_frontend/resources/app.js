( function() {
		...
		*/
		...
		var onCheck = function(event) {
			...
			if (list == 'todo') {
				lists.done.appendChild(task);
				addToDone(new XMLHttpRequest, "http://127.0.0.1:8080/todo_app/addtodone", task.id);
				this.checked = true;
			} else {
				lists.todo.appendChild(task);
				addToTodo(new XMLHttpRequest, "http://127.0.0.1:8080/todo_app/addtotodo", task.id);
				this.checked = false;
			};
		...
		var onRemove = function(event) {
			...
			if (list == 'todo') {
				removeTodo(new XMLHttpRequest, "http://127.0.0.1:8080/todo_app/removetodo", task.id);
			} else {
				removeTodo(new XMLHttpRequest, "http://127.0.0.1:8080/todo_app/removefromdone", task.id);
			};
		};
		...
		var onInput = function() {
			...
			if (tNameValue.length > 0 && tDescriptionValue.length > 0) {
				addNewTodo(new XMLHttpRequest, "http://127.0.0.1:8080/todo_app/addnewtodo", tNameValue, tDescriptionValue);
				todo_name.value = '';
				todo_desc.value = '';
			};
			todo_name.focus();
		...
		var main = function() {
			addEventListeners();
			getTodos(new XMLHttpRequest(), "http://127.0.0.1:8080/todo_app/todosasjson");
			getDoneTodos(new XMLHttpRequest(), "http://127.0.0.1:8080/todo_app/donetodosasjson");
			todo_name.focus();
		};
